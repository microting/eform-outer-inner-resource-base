...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("Microting.eFormOuterInnerResourceBase")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microsoft.EntityFrameworkCore.Design'
        ]
        process_repository('Microting.eFormOuterInnerResourceBase.csproj', packages, 'eform-outer-inner-resource-base')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-outer-inner-resource-base -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
